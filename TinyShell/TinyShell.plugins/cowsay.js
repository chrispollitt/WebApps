...
TinyShell.plugins.cowsay = new Class({
	...
	run : function(t, args, line) {
		...
		var cols = 0;
    var i;
		for (i = 0; i < rows; i++) if (text[i].length > cols) cols = text[i].length;
		t.print();
		t.print("  _"+this.repeat("_", cols)+"_ ");
		for (i = 0; i < rows; i++) t.print(" "+(rows<2?"<":!i?"/":i+1<rows?"|":"\\")+" "+text[i]+this.repeat(" ", cols-text[i].length)+" "+(rows<2?">":!i?"\\":i+1<rows?"|":"/"));
