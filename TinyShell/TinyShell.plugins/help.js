...
TinyShell.plugins.help = new Class({
	...
	run : function(t) {
		...
		var plugs = [];
		for (var plugin in TinyShell.plugins) plugs.push(plugin+this.repeat(" ", 19-plugin.length)+" "+$pick(new TinyShell.plugins[plugin]().description, "No description available"));
		plugs.sort();
		t.print(plugs.join("\n"));
		t.resume();
	},
